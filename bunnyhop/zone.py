...
class Zone(base.BaseBunny):
    ...
    def get(self, id):
        ...
        try:
            if response.get('Id', None):
                return ZoneObject(self.api_key,**response)
        except:
    ...
    def create(self, Name, Type, OriginUrl, StorageZoneId=None):
        ...
        response = self.call_api(f"/pullzone", "POST", self.get_header(), json_data=api_data)
    ...
    def list(self):
        return [ZoneObject(self.api_key, **i) for i in self.call_api(f"/pullzone", "GET")]
    ...
    def update(
        ...
        response = self.call_api(f"/pullzone/{id}", "POST", self.get_header(), json_data=api_data)
        if type(response) is bytes:
            return"Successfully updated."
        return response

    def delete(self, id):
        response = self.call_api(f"/pullzone/{id}", "DELETE", self.get_header())
        if type(response) is bytes:
            return "Zone deleted."
        return response
    
    def purge(self, id):
        response = self.call_api(f"/pullzone/{id}/purgeCache", "POST", self.get_header())
        if type(response) is bytes:
    ...
    def create_edge_rule(
        ...
        response = self.call_api(f"/pullzone/{id}/edgerules/addOrUpdate", "POST", self.get_header(), json_data=api_data)
        if type(response) is bytes:
...
class ZoneObject(base.BaseBunny):
    Id = base.IntegerProperty()
    ...
    def delete(self):
        response = self.call_api(f"/pullzone/{self.Id}", "DELETE", self.get_header())
        if type(response) is bytes:
            return "Zone deleted."
        return response
    ...
    def update(
        ...
        response = self.call_api(f"/pullzone/{self.Id}", "POST", self.get_header(), json_data=api_data)
        if type(response) is bytes:
            for key in api_data:
                if api_data[key] is not None:
                    setattr(self, key, api_data[key])
            return"Successfully updated."
        return response
    
    def purge(self):
        response = self.call_api(f"/pullzone/{self.Id}/purgeCache", "POST", self.get_header())
        if type(response) is bytes:
            return "Purge Queue was finished."
        return response
